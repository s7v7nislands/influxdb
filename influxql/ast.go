...
// HasDefaultDB provides an interface to get the default DB from a Statement.
type HasDefaultDB interface {
	Node
	stmt()
	DefaultDB() string
}
...
// DefaultDB returns the default database from the statement.
func (s *CreateContinuousQueryStatement) DefaultDB() string {
	return s.Database
}
